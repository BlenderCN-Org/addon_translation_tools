...
class TemplateGenerateOperator(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            text.translation_tools.addon_name = mod.__name__
            ...
            if len(text.translation_tools.items) == 0:
...
def register():
    try:
        bpy.app.translations.register(translation_id, translation_dict)
    except ValueError:
...
"""
BL_INFO="""
# Please edit bl_info after translation work is completed
bl_info = {
    "name": "${ADDON_NAME} Translation ${LOCALE}",
    "version": (1, 0, 0),
    "author": "",
    "tracker_url": "",
...
class ModuleGenerateOperator(Operator):
    ...
    def execute(self, context):
        ...
        if prop.mode == "Standalone":
            bl_info = BL_INFO.replace("${ADDON_NAME}", prop.addon_name).replace("${LOCALE}", prop.locale)
            template = MODULE_TEMPLATE.replace("${BL_INFO}", bl_info)
        else:
            template = MODULE_TEMPLATE.replace("${BL_INFO}", "")
        template = template.replace("${TRANSLATION_DICT}", translation_dict)
        bpy_text.clear()
